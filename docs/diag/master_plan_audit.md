...
| Блок | Файлы | Характер |
|---|---|---|
| 1 Оверлей | `service/OverlayController.kt` (+isAttached, caller-лог), `service/OverlayService.kt` (watchdog, фон #99000000, FLAG_NOT_FOCUSABLE, touch-лог), `data/SimpleModeController.kt` (убрать промежуточные hide), `ui/MainViewModel.kt`, `ui/MainActivity.kt`, `res/values*/strings.xml` × 7 | 5+ файлов, структурная |
...
11. `hint`/`showManualPointer` — мёртвый код, удалён в коммите overlay continuity.
...
4. Текущий WIP (7 файлов) — коммит `chore: pre-master WIP` перед фиксом
   оверлея; `semantic_steps.json` в него не входит.
5. `SequenceKind` расширен значением `OVERFLOW_MENU` (использован в
   sys_recommendations).
6. 7 тапов по `textVersion` в MenuDialog → `diagLevelOverride`; purge-триггеры:
   старт приложения + `captureAndSaveSnapshot`.
...
- `sequenceKind` ∈ {CONSENT_WALL, DELAYED_CONFIRM, OVERFLOW_MENU};
  enum `SequenceKind` расширен значением `OVERFLOW_MENU`.
- `consentPolicy` на верхнем уровне (welcomeMarkers/permissionMarkers/
  welcomeActions/defaultDecision/allowOverrides).