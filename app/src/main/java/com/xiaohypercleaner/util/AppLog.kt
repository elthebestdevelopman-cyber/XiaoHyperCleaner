...
object AppLog {
    ...
    private val lock = Any()  // ✅ ДОБАВЛЕНО: lock для thread-safety

    fun init(context: Context) {
        if (!logToFile) return

        synchronized(lock) {  // ✅ ДОБАВЛЕНО: синхронизация
            try {
                closeWriter()
    ...
    private fun rotateLogsIfNeeded() {
        // ✅ ДОБАВЛЕНО: вызывается внутри synchronized(lock)
    ...
    private fun writeToFile(level: String, tag: String, msg: String, throwable: Throwable?) {
        synchronized(lock) {  // ✅ ДОБАВЛЕНО: синхронизация записи
    ...
    fun getLogFile(): File? {
        synchronized(lock) {  // ✅ ДОБАВЛЕНО: синхронизация чтения
    ...
    fun close() {
        synchronized(lock) {  // ✅ ДОБАВЛЕНО: синхронизация закрытия
            closeWriter()
            file = null
        }
    }
