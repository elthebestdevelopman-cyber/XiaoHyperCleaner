...
| Рекомендации в папках рабочего стола | Откройте папку на рабочем столе → нажмите на её название (HyperOS 2/3: удерживайте папку → «Изменить папку») → выключите «Рекомендуемое сегодня». |
...
### Unit-тесты (142 теста, 20 сьютов)
...
|-------------------------------|--------|----------------------------------------------------|
| `OptimizationEngineTest`      | 11     | Оптимизация, откат, DNS, транзакции                |
| `RestoreSnapshotTest`         | 3      | Снапшот оригиналов для отката                      |
| `AdbPortResolverTest`         | 5      | mDNS discovery, mergePorts                         |
| `AdaptiveCatalogTest`         | 16     | Вариантный каталог, мердж текстов                  |
| `SemanticCatalogTest`         | 6      | Семантика шагов, пакеты-цели                       |
| `SemanticVariantTest`         | 7      | Выбор вариантов по версии ОС (MIUI 13 / HyperOS 2) |
| `SemanticGateTest`            | 6      | Гейт уверенности                                   |
| `PlanBuilderTest`             | 6      | Префильтр плана + красный список                   |
| `ManualStepsTest`             | 4      | Ручная памятка: паритет 7 локалей                  |
| `SwitchFinderTest`            | 8      | Тумблеры, checked_before, запрет чужой строки      |
| `ConsentWallTest`             | 9      | Диалоги согласия, чекбоксы, dismiss                |
...
| `SimpleStepsTest`             | 5      | Карта шагов Простого режима                        |
